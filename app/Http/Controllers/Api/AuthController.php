...
class AuthController extends Controller
{
    ...
    public function signUpWithMobile(Request $request)
    {
        ...
        $userCount = User::withTrashed()
            ...
            ->count();
        $otp = sendWhatsAppOtp($request->mobile);
        if ($userCount > 0) {
    ...
    public function assignRole(Request $request)
    {
        ...
        if ($validator->fails()) {
            $responseArray = apiResponse('Failed', $validator->errors(), true, '', 422, $tip, 'Validation error');
        ...
        try {
            ...
            if (! $user) {
                $responseArray = apiResponse('Failed', '', true, '', 401, $tip, 'Unauthorized');
            ...
            $getRole = Role::where('role_slug', $request->role_type)->first();

            if (! $getRole) {
                DB::rollBack();
                $responseArray = apiResponse('Failed', '', true, '', 404, $tip, 'Role not found');

                return response()->json($responseArray, 404);
            }

            $user->role_id = $getRole->id;
            $user->save();

            DB::commit();

            $responseArray = apiResponse('Success', $user, false, '', 200, $tip, $message);

            return response()->json($responseArray, 200);

        } catch (\Exception $ex) {
            DB::rollBack();
            $responseArray = apiResponse('Failed', [], true, $ex->getMessage(), 500, $tip, 'Something went wrong');
    ...
    public function getRoles()
    {
        ...
        $getRoles = Role::select('id', 'role_name', 'role_slug', 'role_description')->where('status', 1)
            ->whereNotIn('role_slug', ['super_admin'])
            ->orderBy('id', 'asc')
            ->get();

        $responseArray = apiResponse('Success', '', false, $getRoles, 200, $tip, $message);
        return response()->json($responseArray, 200);
    }
}
